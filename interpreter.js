 ...
 * @constructor
 */
var Interpreter = function(code, opt_initFunc, debug_callbacks) {
  ...
  this.paused_ = false;
  this.polyfills_ = [];
  this.debugCallbacks_ = debug_callbacks;
  this.debugCallback_ = function(_event, _node, _state, _scope) {
    if (this.debugCallbacks_) {
      this.debugCallbacks_(_event, _node, _state, _scope);
    }
  }
...
Interpreter.prototype.initConsole = function(scope) {
  var thisInterpreter = this;
  var myConsole = thisInterpreter.createObjectProto(this.OBJECT_PROTO);
  this.setProperty(scope, 'console', myConsole);
  var functions = ['debug','error','info','log','warn'];
  for (var i = 0; i < functions.length; i++) {
    this.setProperty(myConsole, functions[i],
        this.createNativeFunction(console[functions[i]], false),
        Interpreter.NONENUMERABLE_DESCRIPTOR);
  }
};
...
Interpreter.prototype['stepCallExpression'] = function(stack, state, node) {
  ...
  if (state.doneCallee_ === 1) {
    ...
    state.doneCallee_ = 2;
    this.debugCallback_("Determining callee", node, state);
  ...
  if (!state.doneArgs_) {
    this.debugCallback_("Determining args", node, state);
    if (state.n_ !== 0) {
  ...
  if (!state.doneExec_) {
    ...
    if (funcNode) {
      ...
      if (funcNode.start == undefined) {
        this.debugCallback_("Executing simple call", node, state, scope);
      } else {
        this.debugCallback_("Executing simple call", funcNode, state, scope);
      }
      ...
      if (typeof code !== 'string') {
        ...
        this.debugCallback_("Executing call", node, state, scope);
        this.value = undefined;  // Default value if no code.
        return new Interpreter.State(evalNode, scope);
      }
    } else if (func.nativeFunc) {
      this.debugCallback_("Executing native call", node, state);
      ...
      this.paused_ = true;
      this.debugCallback_("Executing async call", node, state);
    ...
    this.debugCallback_("Call complete", node, state);
...
Interpreter.prototype['stepArrowFunctionExpression'] = function(stack, state, node) {
  stack.pop();
  stack[stack.length - 1].value = this.createFunction(node, state.scope);
}
